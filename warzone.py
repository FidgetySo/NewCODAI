...
class BlobEnv:
    ...
    end=cv2.cvtColor(end,cv2.COLOR_BGRA2GRAY)
    print(ref.shape)
    print(end.shape)
    player = Blob()
    ...
    def step(self, action):
        ...
        done_check_var = self.done_check(img, self.ref)
        end_var=self.end_check(img,self.end)
    ...
    def done_check(self, img, ref):
        img=np.array(img)[...,:3]
        game_frame=img[23:23+81,89:89+229,:]
        ...
        return ssim_score
    def end_check(self, img, ref):
        img=np.array(img)[...,:3]
        game_frame=img[795:795+202,1422:1422+409,:]
        game_frame=cv2.cvtColor(game_frame,cv2.COLOR_BGRA2GRAY)
        print(game_frame.shape)
        ssim_score=skimage.metrics.structural_similarity(
            game_frame,
...
agent = DDQN(11,(env.OBSERVATION_SPACE_VALUES))
